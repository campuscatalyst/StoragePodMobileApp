  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (domainVerification) {
        ...
        const response = await axios.get(`https://${domain}/api/v1/server-status`);

        if (response.error) {
          return reject(response.error);
        }

        if (response.data && response.data.status && response.data.status === "online") {
          return resolve(true);
        }
...
export const downloadFile = (path) => {
  return new Promise(async (resolve, reject) => {
    try {
      const token = await getJWT();
      const response = await axios.post(`${baseURL()}/files/download?path=${path}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const getCompressStatus = (task_id) => {
 return new Promise(async (resolve, reject) => {
    try {
      const token = await getJWT();
      const response = await axios.get(`${baseURL()}/files/compress-progress?task_id=${task_id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
