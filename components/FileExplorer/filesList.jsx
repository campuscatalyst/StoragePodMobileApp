...
import { getJWT, getStorageFolderUri, saveStorageFolderUri } from "~/lib/storage/tokenStorage";
...
export default function FilesList({ data }) {
  ...
  const selectedItemToDeleteSource = useFileExplorerStore((state) => state.selectedItemToDeleteSource);
  const selectedItemToDownload = useFileExplorerStore((state) => state.selectedItemToDownload);
  ...
  const androidDownloadHandler = async (uri) => {
    try {
      ...
      const newFileUri = await FileSystem.StorageAccessFramework.createFileAsync(storageFolderUri, fileName, mimeType);

      console.error(uri);
      console.error(newFileUri);
      await copyToPublicStorage(uri, newFileUri);

      successNotification("File saved to your device's 'StoragePodDownloads' folder.");
    } catch (error) {
      console.error(error);
  ...
  const downloadHandler = async () => {
    ...
    try {
      ...
      if (!selectedItemToDownload || !Object.keys(selectedItemToDownload).length) {
        //TODO - throw an error notification.
        return;
      }
      ...
      const itemPath = `${getPath(selectedFolderPathList)}/${selectedItemToDownload?.name}`.replace(/\/+/g, "/");
  ...
  const saveMediaFile = async (fileUri) => {
    try {
      ...
      successNotification("Media saved to your gallery under 'StoragePodDownloads'.");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      />
      <DownloadItemBottomSheetModal ref={downloadItemBottomSheetRef} progress={downloadProgress} />
      <CompressFolderBottomSheetModal ref={compressFolderBottomSheetRef} />
      <FilePreviewModal showPreviewModal={showPreviewModal} setShowPreviewModal={setShowPreviewModal} previewFile={previewFile} />
      <FlatList
        data={files}
