  ...
  return (
    <View className="flex-1 flex gap-y-4">
      ...
      <View className="flex flex-row items-center justify-between gap-x-4">
        <View
          className="flex-1 p-4 rounded-3xl h-64 w-48 overflow-hidden relative"
          style={{ backgroundColor: "#DAD5C3" }}
        >
          <View className="flex flex-col gap-y-2">
            <Text className="font-bold text-black">Documents</Text>
            <Text className="font-bold text-3xl text-black">
              {humanFormat(getMetrics.data?.documents, { maxDecimals: 1 })}
            </Text>
          </View>
        ...
        >
          <View className="flex flex-col gap-y-2">
            ...
            <Text className="font-bold text-3xl text-black">
              {humanFormat(getMetrics.data?.images, { maxDecimals: 1 })}
            </Text>
          </View>
      ...
      <View className="flex flex-row items-center justify-between gap-x-4">
        <View
          className="flex-1 p-4 rounded-3xl h-64 w-48 overflow-hidden relative"
          style={{ backgroundColor: "#3B2C35" }}
        >
          <View className="flex flex-col gap-y-2">
            <Text className="font-bold text-white">Videos</Text>
            <Text className="font-bold text-3xl text-white">
              {humanFormat(getMetrics.data?.videos, { maxDecimals: 1 })}
            </Text>
          </View>
        ...
        >
          <View className="flex flex-col gap-y-2">
            ...
            <Text className="font-bold text-3xl text-white">
              {humanFormat(getMetrics.data?.audio, { maxDecimals: 1 })}
            </Text>
          </View>
